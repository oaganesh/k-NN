...
//
//    public static final String VECTOR_STATS_EXTENSION = "vstats";
...
//
//    /**
//     * Creates a SegmentProfilerState from a byte array
//     *
//     * @param bytes byte array containing serialized profiler state
//     * @return deserialized SegmentProfilerState
//     */
//    public static SegmentProfilerState fromByteArray(byte[] bytes) throws IOException {
//        try (ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
//             DataInputStream dis = new DataInputStream(bais)) {
//
//            int statsCount = dis.readInt();
//            List<SummaryStatistics> statistics = new ArrayList<>(statsCount);
//
...
/**
 * Handles profiling and statistical analysis of KNN vector segments
 */
@Log4j2
public class SegmentProfilerState {
    ...
    public static SegmentProfilerState profileVectors(
        ...
        // Initialize statistics for each dimension
        for (int i = 0; i < dimension; i++) {
    ...
    /**
     * Converts the profiler state to a byte array for storage
     *
     * @return byte array representation of the profiler state
     */
    public byte[] toByteArray() throws IOException {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
             DataOutputStream dos = new DataOutputStream(baos)) {
    ...
    /**
     * Formats a double value to 4 decimal places
     *
     * @param value Double value to format
     * @return Formatted double value
     */
    ...
    public String toString() {
        ...
        sb.append("SegmentProfilerState{dimensions=").append(statistics.size()).append(", stats=[");

        for (int i = 0; i < statistics.size(); i++) {
            if (i > 0) sb.append(", ");
            SummaryStatistics stat = statistics.get(i);
            sb.append(String.format("dim_%d{count=%d, min=%.4f, max=%.4f, mean=%.4f, stddev=%.4f}",
                    i, stat.getN(), stat.getMin(), stat.getMax(), stat.getMean(), stat.getStandardDeviation()));
        }
